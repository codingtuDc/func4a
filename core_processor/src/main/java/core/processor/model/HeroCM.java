...
import static core.processor.StaticData.CLASS_NAME_PERMISSIONFUNC;
import static core.processor.StaticData.CLASS_NAME_PERMISSIONS;
import static core.processor.StaticData.CLASS_PATH_CODE_4_REQUEST;
import static core.processor.StaticData.CLASS_PATH_HERO;
import static core.processor.StaticData.CLASS_PATH_INTENT;
import static core.processor.StaticData.CLASS_PATH_NET;
import static core.processor.StaticData.CLASS_PATH_PASS;
import static core.processor.StaticData.CLASS_PATH_PERMISSIONFUNC;
import static core.processor.StaticData.CLASS_PATH_PERMISSIONS;
import static core.processor.StaticData.CLASS_PATH_RESPONSE;
import static core.processor.StaticData.CLASS_PATH_VIEW;
...
public class HeroCM extends ClassModel {
    ...
    public HeroCM(Elements elementUtils, TypeElement te, String prex) {
        ...
        addLines(importLinesIndex, "import " + CLASS_PATH_HERO + ";\r\n");
    ...
    public void addClick(ExecutableElement ee) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (clickTag != null) {
                ...
                if (CLASS_PATH_VIEW.equals(type)) {
    ...
    public void addResult(ExecutableElement ee) {
        ...
        if (!isDeal) {
            if (count != 3) {
                ...
                if (!int.class.getName().equals(parameters.get(0).asType().toString())) {
                    return;
                }
                if (!int.class.getName().equals(parameters.get(1).asType().toString())) {
                    return;
                }
                if (!CLASS_PATH_INTENT.equals(parameters.get(2).asType().toString())) {
                    return;
                }
            }
        }
        ...
        addImport(CLASS_PATH_CODE_4_REQUEST);

        addLines(resultLinesIndex, "        if (requestCode == " + CLASS_NAME_CODE_4_REQUEST + "." + staticName + ") {\r\n");
        addLines(resultLinesIndex, "            if (resultCode == Activity.RESULT_OK) {\r\n");
        addLines(resultLinesIndex, "                this.binder." + methodName + "(");

        if (isDeal) {
            addImport(CLASS_PATH_PASS);
            Ls.ls(parameters, new Each<VariableElement>() {
                @Override
                public boolean each(int position, VariableElement ve) {
                    if (position == count - 1) {
                        addLines(resultLinesIndex, CLASS_NAME_PASS + "." + ve.getSimpleName().toString() + "(data)");
                    } else {
                        addLines(resultLinesIndex, CLASS_NAME_PASS + "." + ve.getSimpleName().toString() + "(data), ");
                    }
                    return false;
    ...
    public void addAccept(ExecutableElement ee) {
        ...
        if (Void.class.getName().equals(netBackValue)) {
            ...
            if (!CLASS_PATH_RESPONSE.equals(parameters.get(1).asType().toString())) {
                return;
            }
    ...
    public void addPermission(ExecutableElement ee) {
        addImport(CLASS_PATH_PERMISSIONS);

        String methodName = ee.getSimpleName().toString();
        String checkIdName = StringFunc.getStaticName(methodName);

        addLines(permissionLinesIndex, "        if (requestCode == " + CLASS_NAME_PERMISSIONS + "." + checkIdName + ") {\r\n");
        ...
        for (int i = 0; i < count; i++) {
            ...
            if ("boolean".equals(typeStr)) {
                addImport(CLASS_PATH_PERMISSIONFUNC);
                addLines(permissionLinesIndex, CLASS_NAME_PERMISSIONFUNC + ".allow(grantResults)");
